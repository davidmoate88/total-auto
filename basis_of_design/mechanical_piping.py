    ...
    return MechanicalPipingBasisOfDesign(
        ...
        pipe_sizing_and_flow=BasisOfDesignSection(
            ...
            calculations_required=[
                CalculationRequirement(
                    name="Line sizing / velocity check",
                    description="Actual velocity from flow rate/internal diameter, checked against the API RP 14E erosional velocity limit and a target velocity range, calcs/mechanical_piping/line_sizing_velocity_check.py. Velocity/erosion only -- pressure drop is not calculated, and actual internal diameter is a required direct input (ASME B36.10M), not derived -- see that module's docstring.",
                    standard_reference="API RP 14E",
                    calc_module_reference="mechanical_piping_line_sizing_velocity_check",
                ),
            ],
            criteria=[
